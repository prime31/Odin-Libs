...
prepper :: proc() {
	...
	fna.set_vertex_buffer_data(device, vbuff, 0, &vertices, len(vertices) * size_of(Vertex), .None);
	ibuff = fna.gen_index_buffer(device, 0, .Write_Only, 6, ._16_Bit);
	fna.set_index_buffer_data(device, ibuff, 0, &indices, 6, .None);

	// load an effect
	data, success := os.read_entire_file("assets/Dots.fxb");
...
create_window :: proc() -> ^sdl.Window {
	...
	sdl.gl_set_attribute(sdl.GL_Attr.Context_Major_Version, 4);
